...
working_dir = "/Users/meghana/Documents/projects/AutoKeras"
...
training_data_path = "/Users/meghana/Documents/projects/AutoKeras/3000.csv"

model_save_file = "/Users/meghana/Documents/projects/AutoKeras/auto_try.keras"
...
def clean_and_convert(df, column_name):
    try:
        # Replace any non-numeric values with -999.0
        df[column_name] = df[column_name].apply(lambda x: -999.0 if not isinstance(x, (int, float)) else x)
        # Convert the column to numeric
        df[column_name] = pd.to_numeric(df[column_name], errors='coerce')
        print(f"Successfully cleaned and converted column: {column_name}")
    except Exception as e:
        print(f"Error cleaning and converting column {column_name}: {e}")
...
def get_data():
    ...
    target_col = 'FRP'
    existing_df = pd.read_csv(training_data_path, low_memory=False)

    # Identify columns with mixed types
    mixed_type_columns = identify_mixed_type_columns(existing_df)
    print(f"Columns with mixed types: {mixed_type_columns}")

    # Clean and convert columns with mixed types
    for col in mixed_type_columns:
        clean_and_convert(existing_df, col)
